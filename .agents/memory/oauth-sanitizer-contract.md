...
3a. **Факт наличия секрета — булев флаг `has<Key>`, а не значение.** `sanitizeOAuthSecrets`, вырезая непустой секрет, оставляет вместо него `hasToken`/`hasAccessToken`/`hasBotToken`/… — по образцу `sanitizeFacebookAccount.hasAccessToken`. Это НЕ плейсхолдер: флаг живёт рядом с полем, в само поле токена по-прежнему ничего не подставляется. Нужен, чтобы UI мог сказать «сохранено» и не врать пустым полем (`telegram.chatId` для этого не годится: chatId сохраняется отдельно, состояние «chatId без токена» достижимо). Флаги вычисляются на чтении и **в базе не хранятся** — `mergeOAuthSettings` выкидывает их из входящего PATCH и подчищает уже просочившиеся, иначе флаг разойдётся с реальным секретом и снова начнёт врать.
...
- **Грабля 3 (2026-07-26):** починили бейджи в настройках кампании, но пропустили панель публикации — `getConnectedPlatforms` в `client/src/pages/content/index.tsx` проверял `instagram.accessToken`, `youtube.accessToken`, `threads.accessToken`. Все три вырезаны, поэтому платформы вечно показывались «Не подключено» при рабочих настройках. Лечение: правила вынесены в **`client/src/lib/platform-connection.ts`** — единственный источник правды, им пользуются и настройки, и панель публикации. **Новую проверку «подключено» писать только там.**
