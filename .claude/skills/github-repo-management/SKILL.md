...
description: "Complete GitHub operations: repo management, authentication, code review, issues, and PR workflows via gh CLI or curl REST API."
version: 2.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [GitHub, Repositories, Git, Releases, Secrets, Configuration, Code-Review, Pull-Requests, Issues, Authentication]
...
## GitHub Authentication (absorbed from `github-auth`)
...
2. `gh` installed but not authenticated → `gh auth login`
3. `gh` not installed → git-only method (no sudo needed)

### Git-Only: HTTPS with PAT

```bash
git config --global credential.helper store
# On next git operation, enter: username + PAT (not password)
...
# Add pubkey to https://github.com/settings/keys
...
**Full script:** `scripts/gh-env.sh` — detects auth method, extracts tokens.

---

## Code Review (absorbed from `github-code-review`)
...
git diff main...HEAD | grep -n "print(\|console\.log\|TODO\|FIXME"  # debug left-overs
...
```bash
gh pr view 123 && gh pr diff 123     # gh
# or checkout locally:
...
curl -s -X POST -H "Authorization: token $GITHUB_TOKEN" \
  ...
  -d "{\"body\":\"...\",\"path\":\"file.py\",\"commit_id\":\"$HEAD_SHA\",\"line\":45,\"side\":\"RIGHT\"}"
```
...
## Issues Management (absorbed from `github-issues`)

### Create Issue

```bash
gh issue create --title "Bug: ..." --body "## Steps to Reproduce\n1..." --label "bug" --assignee "user"
# or curl POST /repos/$OWNER/$REPO/issues
```
...
## PR Workflow (absorbed from `github-pr-workflow`)
...
2. `gh run view <ID> --log-failed` — read error
3. Fix with `patch`/`write_file`
4. `git add -A && git commit -m "fix: ..." && git push`
5. Re-check CI (up to 3 attempts)
...
**References:** `references/conventional-commits.md`, `references/ci-troubleshooting.md`

---

## Pitfalls

### Obsidian vault conflicts (`.obsidian/workspace.json`)

When syncing a vault that contains `workspace.json`, you will frequently hit:
...
1. If you have no local workspace changes you care about:
   ```bash
   git checkout -- .obsidian/workspace.json
   git pull --rebase origin main
   ```

2. If you do care about local workspace state:
   ```bash
   git stash --include-untracked
   git pull --rebase origin main
   # If stash pop fails because the file conflicted:
   git checkout -- .obsidian/workspace.json
   git stash pop
   ```

3. If you end up in a broken rebase state:
   ```bash
   git rebase --abort
   git checkout -- .obsidian/workspace.json
   git pull --rebase origin main
...
**Rule:** `workspace.json` is safe to discard/overwrite because it is UI state only. Treat it as a conflicted cache file, not source of truth.
