...
7. **Owner (Dmitry)** — приоритеты и решения по спорным местам. Деплой руками у него не запрашивается: это зона Claude. Профиль: `docs/agents/owner.md` (рабочая модель, классы инженерных задач, что уходит в owner-gate).
...
| **Codex** | Ревью. |
| **Owner (Dmitry)** | Приоритеты, спорные трейд-офы, ротация кредов, включение флагов, QA на живом проде. Профиль: `docs/agents/owner.md` (охватывает все 44 репозитория владельца, не только этот) |
