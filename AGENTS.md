...
4. **Claude** коммитит, пушит в `origin/main` и выкатывает прод одной командой `scripts/deploy-smm.sh` — см. `docs/DEPLOYMENT.md`. Ручные `docker compose build/up` и `docker cp dist` больше не канон: первый теперь молча ничего не собирает (exit 0, контейнер перезапускается на прежнем образе), второй оставляет в контейнере код, которого нет ни в одном образе. Оба случая выглядят как успешная выкатка — сверяйте `revision`.
...
# docker compose build smm — НЕ канон. Он НЕ падает: выходит с кодом 0 и молча
# ничего не собирает, потому что секции build: у сервиса нет. Связка build + up
# перезапустит контейнер на ПРЕЖНЕМ образе и будет выглядеть успешной.
# Единственная проверка — сверить revision с origin/main.
# Канон блока compose — docs/deploy/compose-smm.fragment.yml
```
