...
- **`docs/prompts/`** - ТЗ для агентов-исполнителей (Codex/Kimi/Claude/Mavis) и конвенции зоопарка. Включает:
  - `TESTING_AS_DOCUMENTATION.md` — **конвенция: тесты как объяснители** (6 правил: имя=контракт, regression-anchor, пример=документация, комментарии за намерением, инвариант через хелпер, рядом с существующим). Чек-лист применяется при приёмке тестов второй парой глаз.
