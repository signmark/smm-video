...
// Подключения кампании компонент читает тем же запросом, что и страница
// кампании, — через api.campaigns.get. Его и подменяем.
vi.mock('../../lib/api-client', () => ({
  api: { campaigns: { get: vi.fn(async () => ({ data: null })) } },
}));
...
beforeEach(() => {
  ...
  vi.mocked(api.campaigns.get).mockReset();
  vi.mocked(api.campaigns.get).mockImplementation(async () => ({ data: null }) as any);
});
...
describe('SM-18: warning о неподключённой сети в промте', () => {
  ...
  it('НЕ показывается, если все упомянутые сети подключены', () => {
    ...
    vi.mocked(api.campaigns.get).mockReset();
    vi.mocked(api.campaigns.get).mockImplementation(async () => ({
      data: {
    ...
    // Ждём именно разрешения запроса, а не «немножко времени»: с setTimeout(0)
    // тест проходил бы и в том случае, когда данные не пришли вовсе.
    return (async () => {
      await waitFor(() => expect(api.campaigns.get).toHaveBeenCalled());
      await waitFor(() =>
        expect(screen.queryByTestId('warning-disconnected-platforms')).toBeNull(),
      );
    })();
  ...
  it('исчезает после подключения сети — без перезагрузки страницы', async () => {
    // Критерий 4. Компонент не перемонтируется: меняются только данные в кэше,
    // ровно как это делает соседняя секция настроек соцсетей после сохранения.
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    ...
    render(
      <Wrapper>
        <AutonomousSettings
          {...defaultProps}
          initialSettings={{ ...defaultInitialSettings, globalPrompt: 'Пиши в Telegram' }}
        />
      </Wrapper>,
    );

    // Сначала Telegram не подключён — предупреждение видно.
    expect(await screen.findByTestId('warning-disconnected-platforms')).toBeInTheDocument();

    // Пользователь подключил Telegram в секции выше; страница инвалидирует
    // запрос кампании по тому же ключу.
    vi.mocked(api.campaigns.get).mockImplementation(async () => ({
      data: { social_media_settings: { telegram: { chatId: '-100123', hasToken: true } } },
    }) as any);
    await qc.invalidateQueries({ queryKey: ['/api/proxy/campaign', defaultProps.campaignId] });
