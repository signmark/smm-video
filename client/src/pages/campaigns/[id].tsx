    ...
    <div className="p-6">
      ...
      <Accordion type="single" defaultValue="site">
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              {getSectionCompletionStatus().publication.completed ? (
                <CheckCircle className="h-5 w-5 text-green-500" />
              ) : (
                <Circle className="h-5 w-5 text-gray-400" />
              )}
              <span>Публикация и постинг</span>
              <span className="text-sm text-muted-foreground ml-auto">
                {getSectionCompletionStatus().publication.label}
              </span>
            </div>
          </AccordionTrigger>
          <AccordionContent className="px-6 pt-2 pb-4">
            <div className="space-y-4">
              <div className="flex flex-col">
                ...
                <p className="text-muted-foreground mt-1 mb-3">
                  Здесь вы можете планировать время выхода ваших постов и отслеживать их статус в социальных сетях.
                </p>
              </div>

              {campaignContent && campaignContent.length > 0 ? (
                <div className="grid gap-4">
                  <p className="text-sm font-medium">Последний созданный контент:</p>
                  <div className="space-y-2">
                    {campaignContent.slice(0, 3).map((item) => (
                      <div key={item.id} className="flex items-center justify-between p-3 border rounded-lg bg-card">
                        <div className="flex flex-col">
                          <span className="font-medium text-sm">{item.title || "Без заголовка"}</span>
                          <span className="text-xs text-muted-foreground truncate max-w-[200px]">
                            {item.status === 'draft' ? 'Черновик' : item.status === 'scheduled' ? 'Запланировано' : 'Опубликовано'}
                          </span>
                        </div>
                        <Button
                          size="sm"
                ...
                <div className="text-center py-6 border-2 border-dashed rounded-lg">
                  <p className="text-muted-foreground">Сначала сгенерируйте контент в разделе выше</p>
                </div>
              )}
            </div>
          </AccordionContent>
