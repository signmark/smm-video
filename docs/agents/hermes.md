...
2. Этот файл + `docs/agents/codebase-map.md` (где что лежит, порядок middleware, поток публикации, метод диагностики — прочитать целиком в первую сессию, дальше по нужде).
