...
1. **Первый приоритет:** `.env` переменные (стандартная практика — env всегда выигрывают над БД)
2. **Второй приоритет:** Глобальные ключи из Directus (можно менять без перезапуска)
3. **Третий приоритет:** Пользовательские ключи из БД
4. **Четвёртый приоритет:** Vertex AI (.env)

**Важно:** Ключи из `.env` используются ВСЕГДА, включая production. Приоритет .env над БД позволяет админу быстро ротировать ключи через правку .env + рестарт.
