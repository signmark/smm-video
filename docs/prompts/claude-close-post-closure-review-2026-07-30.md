...
Jira веди по `/root/.config/jira/README.md`: не комментируй и не меняй статусы по ходу. После push, deploy и live-check добавь в `AI-48` один комментарий «готово к ревью» со всеми пятью обязательными частями из README: точный диапазон `<baseline>..<HEAD>`, коммит и суть находки по каждому пункту, red-before доказательства с файлами/цифрами, результаты четырёх проверок цифрами и явный список того, что не сделано. До вердикта Codex не переводи задачу в «Готово». Если после передачи понадобятся правки, исходный комментарий не редактируй: добавь короткое уточнение с причиной, коммитом и новым диапазоном. `AI-33`/`AI-34` обновляй только если они фактически закрыты и проверены на production.
