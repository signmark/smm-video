...
### 3.5. Прямая публикация (текущий подход)

**Все платформы публикуются напрямую** через собственные сервисы:

| Платформа | Сервис | Метод |
|-----------|--------|-------|
| Telegram | `telegram-service.ts` | Telegram Bot API |
| VK | `vk-service.ts` | VK API |
| Instagram | `instagram-service.ts` | Instagram Graph API |
| Facebook | `facebook-service.ts` | Facebook Graph API |
| YouTube | `youtube-video-service.ts` | YouTube Data API v3 |
| TikTok | `tiktok-service.ts` | TikTok API |
| Threads | `threads-service.ts` | Threads API |

**Устаревшее (n8n):** Ранее публикация шла через N8N webhooks. Сейчас n8n удалён, метод `publishThroughN8nWebhook` помечен как `@deprecated` и выбрасывает ошибку.
