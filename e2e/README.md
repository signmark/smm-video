...
Браузер НЕ ставится обычным `npm ci` — иначе ~110 МБ уезжали бы в каждую сборку
прод-образа, который и так великоват (AI-42). Ставится отдельно и только там,
где гоняются тесты:

```bash
npx playwright --version    # должно совпасть с package-lock.json
npm run e2e:install         # поставит chromium под версию пакета
```

`e2e/run.sh` вызывает `npm run e2e:install` сам, поэтому вручную это нужно
только при отладке. Установка идемпотентна: если нужная сборка уже есть, шаг
занимает секунды.

**Почему шаг в скрипте, а не только в этом файле.** 31.07.2026 прогон E2E упал
целиком: пакет `@playwright/test@1.60.0` требовал сборку `headless_shell-1200`,
а в кеше лежала только `chromium-1223` от прежней установки — setup с логином не
стартовал, 195 тестов не запускались. Документацию забывают, шаг в раннере — нет.
