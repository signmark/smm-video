...
- **video-app/campaigns-proxy**: использует admin-токен + фильтр filter[user_id][_eq]=${userId} — именно так делает основной app. Пользовательский токен возвращает 403 на коллекцию user_campaigns (нет прав на прямое чтение)
