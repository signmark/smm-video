...
# test:dates:msk — те же файлы, что и в основном прогоне, но во ВТОРОМ часовом
# поясе. Прод и CI гоняют тесты в UTC, и там проверка, завязанная на смещение,
# может выродиться: 06.08 ассерт про разницу с new Date() сравнивал ноль с нулём
# и был зелёным, хотя знак в нём был перепутан — поймали случайно, на машине с
# МСК. Полный набор во втором поясе гонять незачем (это +3 минуты на выкатку),
# а пять date-файлов стоят секунды.
#
# calendar-drag-moscow-time сюда НЕ входит осознанно: он сам в шапке объявляет,
# что гоняется при TZ=UTC (расхождение с Москвой три часа) и проверяет именно
# этот сценарий. Под МСК он падает по построению, а не из-за дефекта.
SMM_PREFLIGHT_STEPS="${SMM_PREFLIGHT_STEPS:-check check:client test:run test:dates:msk}"
