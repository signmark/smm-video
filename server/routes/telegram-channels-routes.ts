...
Твоя задача: для каждой фразы придумать 2-4 коротких поисковых слова (1-2 слова), которые связаны с исходной темой.
- Синонимы и близкие понятия (например "маркетинг" → "реклама", "продвижение", "таргет")
- Английские термины если они распространены (например "AI", "no-code")
- НЕ генерируй совсем широкие темы — оставайся в рамках исходной тематики
- НЕ повторяй исходные фразы целиком
- Верни ТОЛЬКО список слов через запятую, без пояснений

Пример: если исходное "SMM-продвижение", ответ: "маркетинг, реклама, таргет, SMM"`;
