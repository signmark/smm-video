...
export async function filterByRelevance<T extends ChannelCandidate>(
  ...
  const prompt = `Ты — строгий эксперт по подбору Telegram-каналов для SMM-кампании.

КЛЮЧЕВЫЕ СЛОВА КАМПАНИИ (ОБЯЗАТЕЛЬНО ТОЧНОЕ СОВПАДЕНИЕ):
${campaignKeywords.map(k => `- ${k}`).join('\n')}

НАЙДЕННЫЕ ${platform === 'telegram' ? 'КАНАЛЫ' : 'ГРУППЫ'}:
${channelList}

ЗАДАЧА: Определи, какие из найденных каналов ТОЧНО соответствуют ключевым словам кампании.
- Канал ТОЛЬКО если его основная тематика ТОЧНО совпадает с ключевыми словами
- НЕ включай каналы которые "связаны" или "смежные" — только ТОЧНОЕ совпадение
- НЕ включай泛流量 каналы (новости, юмор, развлечения) даже если у них много подписчиков
- Если сомневаешься — НЕ включай (лучше меньше каналов но точных)

Верни ТОЛЬКО номера релевантных каналов через запятую. Без пояснений.
Пример: 1,3,5,7

Если ни один не точно соответствует, верни: 0`;
  ...
  try {
    ...
    log(`[AI Relevance] ⚠️ Ошибка AI-фильтрации: ${err.message}. Возвращаем пустой список (безопаснее).`, 'warn');
    return { relevant: [], filtered: candidates, rawAiResponse: `error: ${err.message}` };
  }
}
