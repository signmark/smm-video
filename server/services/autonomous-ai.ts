...
/**
 * AI-130. Отмечает ДОШЕДШИЙ ДО КОНЦА цикл — близнец noteAbortedCycle.
 *
 * ЧТО БЫЛО. Прерывание сохранялось сразу (см. ниже), а успех менял счётчики
 * только в памяти процесса. Из-за этого в базе оставалась отметка предыдущего
 * цикла, и после перезапуска планировщик считал остаток интервала от неё:
 * отметка старше интервала — значит до цикла минимальные пять секунд, то есть
 * выкатка запускала цикл почти сразу и публиковала лишний пост в живые каналы.
 * При девяти выкатках в день это девять лишних постов вместо одного за сутки.
 *
 * ВТОРОЕ. Правило AI-123 «три прерывания ПОДРЯД» держалось на том, что успех
 * обнуляет счётчик. Обнуление тоже жило только в памяти, поэтому после
 * перезапуска возвращалось старое число, и между «подряд идущими» прерываниями
 * могли быть успешные циклы.
 *
 * Работает по campaignId, а не по ссылке на состояние: за время цикла режим
 * могли остановить, и писать копию остановленного режима обратно нельзя —
 * он воскреснет при следующем перезапуске.
 */
export function noteCompletedCycle(campaignId: string): void {
  const state = autonomousStates.get(campaignId);
  if (!state) return;
...
Ответь ТОЛЬКО списком через запятую без пробелов (пример: telegram,vk):`;
    ...
    // AI-130. Счётчики, отметка времени и сохранение — одним местом, общим с
    // веткой прерывания. Раньше здесь стояли те же присваивания без сохранения,
    // и база оставалась с отметкой предыдущего цикла.
    noteCompletedCycle(state.campaignId);
