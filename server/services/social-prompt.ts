...
import {
  PLATFORM_LITERAL_NAMES,
  isNegatedBefore,
  wholeWordRegex,
} from '@shared/social-platform-names';

/**
 * Отображаемые названия платформ для раскрытия `[socialNetworks]`.
 *
 * Выводятся из общего списка, а не задаются вторым словарём: первое написание
 * каждой платформы и есть её основное имя. Раньше здесь лежала копия, и она
 * разошлась с клиентской — клиент не знал ни `TikTok`, ни алиаса `VK`.
 */
export const PLATFORM_NAMES_RU: Record<string, string> = PLATFORM_LITERAL_NAMES.reduce(
  (acc, { key, name }) => {
 ...
 *  Список и правило поиска — общие с клиентом, см. @shared/social-platform-names. */
const LITERAL_NAME_PATTERNS: Array<{ key: string; re: RegExp }> = PLATFORM_LITERAL_NAMES.map(
  ({ key, name }) => ({ key, re: wholeWordRegex(name) }),
);
