...
class PublicationStatusChecker {
  ...
  private tokenValidationIntervalMs = 6 * 60 * 60 * 1000; // каждые 6 часов
  ...
  start() {
    ...
    // Валидация токенов отключена — уведомления о невалидных токенах не нужны
    log('Периодическая валидация токенов отключена', 'status-checker');
  ...
  /**
   * Проверяет все токены во всех кампаниях
   */
  private async checkAllTokens() {
    try {
      log('Запуск плановой валидации всех токенов...', 'status-checker');
      const campaigns = await directusCrud.list('user_campaigns', { useAdminToken: true });
      for (const campaign of campaigns) {
        await statusValidator.validateCampaignTokens((campaign as any).id);
      }
      log('Плановая валидация токенов завершена', 'status-checker');
    } catch (error: any) {
      log(`Ошибка при плановой валидации токенов: ${error.message}`, 'status-checker', 'error');
    }
  }
