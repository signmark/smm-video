...
export type VkErrorKind =
  ...
  | 'auth_expired'
  /**
   * Заблокировано VK-приложение, которым выдан токен.
   *
   * Сам токен при этом может быть «жив», но работать не будет. Лечится
   * переподключением: интеграция идёт через needanapp, и новый токен выпишет
   * его приложение. На 29.07.2026 код 8 отдают 11 кампаний с токенами,
   * выданными ДО перехода на needanapp (у них нет ни clientId, ни
   * tokenReceivedAt) — это наследство, а не поломка текущей интеграции.
   */
  | 'app_blocked'
...
const MESSAGES: Record<VkErrorKind, string> = {
  ...
  app_blocked: 'Токен выдан VK-приложением, которое ВКонтакте заблокировал. Переподключите сообщество — новый токен выпишет рабочее приложение.',
