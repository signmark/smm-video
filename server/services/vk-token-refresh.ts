...
export async function refreshAllExpiringVkTokens(): Promise<void> {
  ...
  try {
    ...
    while (true) {
      ...
      // Рефрешим если токен истекает в ближайшие 8 часов.
      // VK ID v2 токены живут 24 часа, кроп бежит раз в 6 часов.
      // Порог должен быть > 6ч иначе токен никогда не попадёт в окно:
      // при 6ч интервале остаток будет [24, 18, 12, 6] — всё > 4ч.
      // С порогом 8ч: при 6ч остатке (6 < 8) → обновляем. Математика сходится.
      const threshold = Date.now() + 8 * 60 * 60 * 1000; // now + 8 часов
