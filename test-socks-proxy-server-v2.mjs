 ...
 * 
 * Запуск: node test-socks-proxy-server-v2.mjs [API_KEY]
 */
...
// Создаем агента для прокси - нужно форматировать как URL
...
console.log(`Используем URL прокси (username/password скрыты): socks5://***:***@${PROXY_HOST}:${PROXY_PORT}`);
...
async function runTests() {
  ...
  if (apiKey) {
    ...
    console.log('node test-socks-proxy-server-v2.mjs YOUR_API_KEY');
  }
}
