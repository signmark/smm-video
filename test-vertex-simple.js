...
// Простой тест для проверки Vertex AI
    ...
    if (accessTokenResponse.token) {
      ...
      const body = {
        contents: [{
          role: "user",
          parts: [{
            text: 'Проанализируй настроение комментария: "Отличный пост! Мне очень понравилось!" Ответь в JSON формате: {"sentiment": "positive/negative/neutral", "confidence": 85}'
          }]
        }],
        generationConfig: {
          temperature: 0.2,
      ...
      if (response.ok) {
        console.log('🎉 Vertex AI работает! Полный ответ:');
